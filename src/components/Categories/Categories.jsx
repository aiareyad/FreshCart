...
<div className="d-flex justify-content-between flex-wrap margin">
  {data?.data.data.map((cat, idx) => {
    return (
      <div 
        className="w-31 border m-3 flex-grow-1 flex-xl-grow-0 d-flex flex-column" 
        key={idx}
        style={{ height: '500px' }} 
      >
        ...
        <h3 className="text-center text-main my-4">
          {cat.name}
        </h3>
      </div>
    );
