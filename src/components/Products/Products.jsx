  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <div className="row ">
          <form action="" >
            <div className="text-center w-75 mx-auto my-5 d-flex justify-content-center align-items-center">
          ...
          {data?.data.data.map((product) => {
            ...
            return (
              <div key={product.id} className="col-md-6 col-xl-3 product g-4 p-2 mb-3 rounded-3">
                ...
                <button onClick={() => addProToCart(product.id)}className="btn bg-main text-light w-75 m-3">
                  Add to cart
                </button>
                    <button style={{ color: wishListIds?.includes(product.id) ? 'red' : 'black' }}
                      onClick={()=>addToWL(product.id , product)} className={ `heart-btn m-3`}>
